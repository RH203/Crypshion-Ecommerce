              ...
              <tbody>
                @foreach ($products as $product)
                  <tr class="odd:bg-white even:bg-gray-100 hover:bg-gray-100 ">
                    ...
                    <td>
                      <p
                        class="inline-block px-2 text-xs text-center {{ $product->status == 'Waiting' ? 'text-black bg-yellow-500' : '' }}{{ $product->status == 'Packaged' ? 'text-white bg-primaryBg' : '' }}{{ $product->status == 'Delivered' ? 'text-white bg-black' : '' }}{{ $product->status == 'Completed' ? 'text-white bg-green-600' : '' }} rounded-full">
                        {{ $product->status }}</p>
                    </td>
                    <td>
