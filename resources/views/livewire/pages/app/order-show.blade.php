      ...
      <div class="p-5 bg-white shadow-lg rounded-xl basis-8/12">
        <div class="flex items-center justify-between">
          ...
          @foreach ($customerData as $item)
            <p
              class="px-2 text-xs {{ $item->status == 'Waiting' ? 'text-black bg-yellow-500' : '' }}{{ $item->status == 'Packaged' ? 'text-white bg-primaryBg' : '' }}{{ $item->status == 'Delivered' ? 'text-white bg-black' : '' }}{{ $item->status == 'Completed' ? 'text-white bg-green-600' : '' }}{{ $item->status == 'Canceled' ? 'text-white bg-red-600' : '' }}{{ $item->status == 'Confirmed' ? 'text-white bg-purple-600' : '' }} rounded-full">
              {{ $item->status }}</p>
          @endforeach
        </div>
