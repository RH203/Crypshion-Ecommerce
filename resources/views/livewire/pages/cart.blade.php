                ...
                @foreach ($deliveries as $delivery)
                  <div class="w-full">
                    ...
                    <label for="{{ $delivery->id }}"
                      class="block w-full p-3 border-2 rounded-lg border-label checked:border-blue-600 hover:cursor-pointer">
                      <p class="text-lg font-semibold">{{ $delivery->name }}</p>
                      <p class="text-xs text-slate-500">Estimation : {{ $delivery->estimation }} day
                      </p>
                      <p class=" text-slate-500">Rp.
                        {{ number_format($delivery->cost, 0, ',', '.') }}</p>
                    </label>
                  </div>
                @endforeach
              </div>
            ...
            @foreach ($datas as $data)
              <div class="flex items-center justify-between my-5">
                <div class="flex">
                  ...
                  <div class="basis-7/12 md:basis-9/12 text-slate-800 ms-5">
                    <div class="w-10/12 mb-1 md:w-10/12">
                      <h4 class="block font-semibold text-md lg:hidden">
                        {{ Str::limit($data->product->title, 30) }}
                      </h4>
                      <h4 class="hidden font-semibold text-md lg:block">
                        {{ Str::limit($data->product->title, 80) }}
                      </h4>
                    </div>
                    <p class="text-xs md:text-sm text-slate-500">{{ $data->product->category->title }}</p>
                    <div>
                      <span
                        class="inline-block px-5 py-0 text-xs border md:text-sm md:px-7 text-slate-500 border-slate-500">{{ $data['size'] }}</span>
                      <span
                        class="inline-block px-5 py-0 text-xs border md:text-sm md:px-7 text-slate-500 border-slate-500">{{ $data->color }}</span>
                    </div>
...
<script type="text/javascript">
  var costP = <?php echo json_encode($total); ?>;
